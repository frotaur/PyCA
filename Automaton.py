...
class CA1D(Automaton):
    ...
    def reset(self, randomize = False):
        ...
        if(not randomize):
            self.world = torch.zeros((self.w),dtype=torch.int)
            self.world[self.w//2]=torch.randint(0,self.k,(1,)) # Random initial state
        else:
...
class GeneralCA1D(Automaton):
    ...
    def __init__(self, size, wolfram_num: int, r=1, k=2, init_state: torch.Tensor = None):
        """
            Parameters :
            size : 2-uple (H,W)
                Shape of the CA world
            ...
            init_state : torch.Tensor of ints between 0 and k-1, shape (W,)
                Initial state of the automaton. If None, it will be a single cell in the middle of the world.
        """
        super().__init__(size)
        self.r = r
        ...
        if(init_state is not None):
            ...
            self.world[self.w//2]=torch.randint(1,k,(1,)) # Random initial state
