from ...automaton import Automaton
import torch, pygame
import torch.nn.functional as F
import colorsys
from .CALearning import FlexiGPT
from torchenhanced import Trainer
from torchvision.transforms import Resize, InterpolationMode


class TensorBuffer:
    def __init__(self, shape, device=None):
        self.buffer = torch.empty(shape, device=device)
        self.T, self.H, self.W = shape
        self.ptr = 0
        self.full = False
        self.num_frames = 0

    def append(self, x):  # x should be (H, W)
        self.buffer[self.ptr] = x
        self.ptr = (self.ptr + 1) % self.T
        self.num_frames = min(self.num_frames+1, self.T)

    def get(self):
        """
           Returns the frame history

           Returns:
              Tensor of shape (T, H, W) containing the last T frames in the buffer
        """
        if self.num_frames < self.T:
            return self.buffer[:self.ptr]
        
        return torch.cat((self.buffer[self.ptr:], self.buffer[:self.ptr]), dim=0)

class DeepCA(Automaton):
    """
    2D Outer-Holistic Cellular Automaton, but which can be switched to run on a deep learning model.
    More precisely, the deep learning model uses the last x frames to predict the next one, and so on.
    """

    def __init__(self, size, deep_model_config_file:str,deep_model_pt_file:str, s_num="23", b_num="3", random=True, device="cpu"):
        """
        Params :
        size : tuple, size of the automaton
        deep_model_config_file : str, the location of the deep learning model config file
        deep_model_pt_file : str, the location of the deep learning model .pt file
        s_num : str, rule for survival
        b_num : str, rule for birth
        random : bool, if True, the initial state of the automaton is random. Otherwise, a small random square is placed in the middle.
        """
        super().__init__(size)

        self.s_num = self.get_num_from_rule(s_num)  # Translate string to number form
        self.b_num = self.get_num_from_rule(b_num)  # Translate string to number form
        self.random = random
        self.device = device

        self.one_in_two = False
        self.flip_flop_draw = True

        config = torch.load(deep_model_config_file, map_location=device)
        config['device'] = device  # Ensure the config has the correct device
        self.model = FlexiGPT(**config).to(device)  # Load the model from the config file
        self.model.load_state_dict(torch.load(deep_model_pt_file, map_location=device))
        self.model.eval()  # Set the model to evaluation mode

        self.T,_,H,W = self.model.in_shape
        self.min_frames = self.T-2  # Minimum number of frames to use the model
        self.internal_h, self.internal_w = H, W  # Internal size of the model

        self.world = torch.zeros((H,W), dtype=torch.int, device=device)
        self.frames_history = TensorBuffer((self.T, H, W), device=device)  # Buffer to store the last T frames
        self.reset()

        self.kernel = torch.tensor([[1, 1, 1], [1, 0, 1], [1, 1, 1]], device=device, dtype=torch.float)[
            None, None, :, :
        ]  # (1,1,3,3) as required by Pytorch

        self.change_highlight_color()
        self.decay_speed = 0.1

        self._worldmap = self._worldmap.to(device)

        self.use_model = True

        self.resizer = Resize((self.h, self.w), interpolation=InterpolationMode.NEAREST)
    def change_highlight_color(self):
        """
        Changes the highlight color, gets random hue
        """
        hue = torch.rand(1).item()
        light = 0.5
        saturation = 0.5
        self.highlight_color = torch.tensor(
            colorsys.hls_to_rgb(hue, saturation, light), dtype=torch.float, device=self.device
        )

    def get_num_from_rule(self, num : str):
        """
        Get the rule number for the automaton, given the string representation of the rule.
        """
        rule_out = 0
        if num != "":  # If the num is empty, the associated number is 0
            rule_out = sum([2 ** int(d) for d in num])

        return rule_out

    def get_rule_from_num(self, num : int):
        """
            Get the rule string from the rule number
        """
        numbers_list = [int(num//2**i % 2) for i in range(9)]

        out = ""
        for i, n in enumerate(numbers_list):
            if n == 1:
                out += str(i)

        return out

    def reset(self):
        """
        Resets the automaton to the initial state.
        """
        self._worldmap = torch.zeros((3, self.h, self.w), device=self.device)

        if self.random:
            self.world = self.get_init_mat(0.5)
        else:
            self.world = torch.zeros_like(self.world, dtype=torch.int, device=self.device)
            self.world[self.internal_h // 2 - 1 : self.internal_h // 2 + 1, self.internal_w // 2 - 1 : self.internal_w // 2 + 1] = torch.randint(
                0, 2, (2, 2), device=self.device
            )
        self.frames_history = TensorBuffer((self.T,self.internal_h,self.internal_w), device=self.device)  # Reset the frame history
        self.frames_history.append(self.world.to(torch.float))  # Append the initial world to the history
    def draw(self):
        """
        Updates the worldmap with the current state of the automaton.
        """
        if(self.one_in_two):
            # If one_in_two is True, we draw only every second frame
            if(self.flip_flop_draw):
                self.flip_flop_draw = not self.flip_flop_draw
                echo = torch.clamp(self._worldmap - self.decay_speed * self.highlight_color[:, None, None], min=0, max=1).to(
                    self.device
                )
                resized_world = self.resizer(self.world[None,None])[0,0] # (H', W')
                self._worldmap = torch.clamp(
                    resized_world[None, :, :].expand(3, -1, -1).to(dtype=torch.float) + echo, min=0, max=1
                ).to(self.device)
            else:
                self.flip_flop_draw = not self.flip_flop_draw

        else:
            echo = torch.clamp(self._worldmap - self.decay_speed * self.highlight_color[:, None, None], min=0, max=1).to(
                self.device
            )
            resized_world = self.resizer(self.world[None,None])[0,0]
            self._worldmap = torch.clamp(
                resized_world[None, :, :].expand(3, -1, -1).to(dtype=torch.float) + echo, min=0, max=1
            ).to(self.device)
        

    def process_event(self, event, camera=None):
        """
        DEL -> re-initializes the automaton
        I -> toggles initialization between noise and a single dot
        N -> pick a new random rule
        Z -> change the highlight color
        D -> toggles the use of the model
        O -> toggle one_in_two drawing (prevents flickering)
        UP -> longer-lasting highlights
        DOWN -> shorter-lasting highlights
        """
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE or event.key == pygame.K_DELETE:
                self.reset()
            if event.key == pygame.K_i:
                self.random = not self.random
            if event.key == pygame.K_d:
                # Toggles the use of the model
                self.use_model = not self.use_model
                if self.use_model:
                    print("Using model")
                else:
                    print("Using classic CA rules")
            if event.key == pygame.K_o:
                # Toggles the one_in_two drawing
                self.one_in_two = not self.one_in_two
                if self.one_in_two:
                    print("One in two drawing enabled")
                else:
                    print("One in two drawing disabled")
            if event.key == pygame.K_n:
                # Picks a random rule
                b_rule = torch.randint(0, 2**9, (1,)).item()
                s_rule = torch.randint(0, 2**9, (1,)).item()
                self.change_num(s_rule, b_rule)
                print(f"rule :  s:{self.get_rule_from_num(s_rule)}, b:{self.get_rule_from_num(b_rule)}")
            if event.key == pygame.K_z:
                self.change_highlight_color()
            if event.key == pygame.K_UP:
                self.decay_speed = max(self.decay_speed - 0.1 * self.decay_speed, 0.005)
            if event.key == pygame.K_DOWN:
                self.decay_speed = min(0.1 * self.decay_speed + self.decay_speed, 3)

    def change_num(self, s_num: int | str, b_num: int | str):
        """
        	Changes the rule of the automaton to the one specified by s_num and b_num
        """
        if(isinstance(s_num, str)):
            s_num = self.get_num_from_rule(s_num)
        if(isinstance(b_num, str)):
            b_num = self.get_num_from_rule(b_num)
            
        self.s_num = s_num
        self.b_num = b_num
        self.reset()

    @torch.no_grad()
    def step(self):
        count = self.convolve_count(self.world)

        if((not self.use_model) or (self.frames_history.num_frames < self.min_frames)):
            # If the model is not used or there are not enough frames, use the classic CA rules
            self.world = torch.where(
                self.world == 1, self.get_nth_bit(self.s_num, count), self.get_nth_bit(self.b_num, count)
            ).to(torch.int)
        else:
            # Use the deep learning model to predict the next frame
            frames = self.frames_history.get() # (T, H, W)
            frames = frames[None,:,None,:,:] # Add batch and channel dimensions (1, T, 1, H, W)
            next_frame = self.model.generate_next_token(frames)
            self.world = next_frame[0,0].to(torch.int)

        self.frames_history.append(self.world.to(torch.float))  # Append the current world to the history


    def convolve_count(self, world):
        """
        Convolves the world with the kernel to get the count of neighbors
        """
        pad_world = F.pad(
            world[None, None, :, :].to(torch.float), (1, 1, 1, 1), mode="circular"
        )  # (1,1,H+2,W+2) circular padded

        return F.conv2d(pad_world, self.kernel)[0, 0, :, :].to(torch.int)  # (H,W)

    def get_nth_bit(self, num, s):
        """
        Get the nth bit of the number num
        """
        return (num >> s) & 1

    def get_init_mat(self, rand_portion):
        """
        Get initialization matrix for CA

        Params :
        rand_portion : float, portion of the screen filled with noise.
        """
        batched_size = torch.tensor([self.internal_h, self.internal_w])
        randsize = (batched_size * rand_portion).to(dtype=torch.int16)  # size of the random square
        randstarts = (batched_size * (1 - rand_portion) / 2).to(
            dtype=torch.int16
        )  # Where to start the index for the random square

        randsquare = torch.where(torch.randn(*randsize.tolist()) > 0, 1, 0)  # Creates random square of 0s and 1s

        init_mat = torch.zeros((self.internal_h, self.internal_w), dtype=torch.int16)
        init_mat[randstarts[0] : randstarts[0] + randsize[0], randstarts[1] : randstarts[1] + randsize[1]] = randsquare
        init_mat = init_mat.to(torch.int16)

        return init_mat.to(self.device)  # (B,H,W)

